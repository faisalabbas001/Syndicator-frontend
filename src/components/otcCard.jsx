...
const OTCcard = ({data}) => {
  ...
  const [selectedChain, setSelectedChain] = useState({chunk_size: "", asset_address: ""});
      ...
      <div className=" flex flex-col flex-1 justify-between">
        <div className="flex px-3 xxl:px-4 pt-3 pb-1 justify-between items-center">
          <div className="flex items-center">
            ...
            <div className="ps-3">
              <h3 className="text-white text-2xl">{ getTokenName(contractAddress.asset_address) }</h3>
              ...
              <span className="text-gray-500 text-xs  rounded-md p-1 custon-gray-bg">
                {sellingAmount.toString() === contractAddress.chunk_size.toString() ? "Entire Fill" : "Partial Fill"}
              </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-between items-center py-1">
          <span>
            ...
            <div className="flex flex-col px-5">
              ...
              <div className="flex items-center mt-1">
                <span className="text-xl me-1 text-gray-300">{sellingAmount.toString() === contractAddress.chunk_size.toString() ? sellingAmount.toString() : formater(sellingAmount)}</span>{" "}
          ...
          {
            forAmount.length > 1 ? <div className=" w-full sm:w-auto">
            <button
            ...
            >
              ...
              <span className="text-sm">{ selectedChain.chunk_size.toString() ? formater(selectedChain.chunk_size) : "" }</span>
          ...
          </div>: <div className="flex items-center"> <img src={getTokenImage(forAmount[0].asset_address)} alt={'Solana'} className="w-5 rounded-full h-5 mr-1"/>{formater(forAmount[0].chunk_size)}</div>
          }
        </div>
        ...
        <div className="border-t border-gray-800 flex justify-between items-center px-6 my-0 py-2 ">
          ...
          <span className="">
            <Link to={`token-sale/bonk`} state={{...data, requested_assets:selectedChain }} className=" bg-gray-600 bg-opacity-35 custom-gray font-bold py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out hover:bg-green-700 hover:text-white">
              <span>Buy</span>
